...
def compute_frames_coupled(t_vect, theta2_vect, d_vect, Zc, Xc, dc,
    ...
    for i in range(N):
        ...
        if Xc < (p4_xz[1] - r_corde) or Xc > (p3_xz[1] + r_corde):
            ...
            # calcul vecteurs
            vect_cc = Cxy - p4_xz  # vecteur du centre corde à p4
            vect_p3c = Cxy - p3_xz  # vecteur du centre corde à p3

            norm_cc = np.linalg.norm(vect_cc)
            norm_p3c = np.linalg.norm(vect_p3c)
            z_p4 = p4_xz[0]  # coord Z de p4
            z_c = Zc          # coord Z du centre corde

            if norm_cc < r_corde:
                # θ_total calculé avec arctan(norm_cc/norm_p3c) + décalage angle selon z/x vect_p3c
                angle_correction = np.arctan2(norm_cc, norm_p3c) + np.arctan2(vect_p3c[0], vect_p3c[1])
                theta_total = angle_correction
                theta3 = theta_total - th2
                theta3 = np.arctan2(np.sin(theta3), np.cos(theta3))  # normalisation angle

                # recalcul p4 avec theta total corrigé
                p4 = p3 + np.array([-D*np.cos(theta_total), 0.0, D*np.sin(theta_total)])
                p4_xz = np.array([p4[2], p4[0]])

                # si z_p4 == z_c alors θ3 = 0 (sortie tangence)
