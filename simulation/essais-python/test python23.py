...
# ------------------------
def compute_frames_coupled(t_vect, theta2_vect, d_vect, Zc, Xc, dc, eps_tang=1e-6, max_iter=50):
    """
    Version géométrique simple basée sur la tangence entre [p3p4] et le cercle.
    (les arguments eps_tang, max_iter sont conservés pour compatibilité mais non utilisés)
    """
    N = len(t_vect)
    frames = []
    ...
    r_corde = dc / 2.0
    C = np.array([Zc, Xc])  # centre du cercle dans le plan (z, x)

    for i in range(N):
        th2 = theta2_vect[i]
        dval = d_vect[i]

        # cinématique directe jusqu’à p3
        ...
        # conditions hors zone de contact
        if Xc < p4_xz[1] - r_corde or Xc > p3_xz[1] + r_corde:
            theta3 = 0.0
            theta_total = th2

        else:
            # Tangence : on ajuste θ_total pour que dist(CC') = dc/2
            vect_p3p4 = p4_xz - p3_xz
            norm_p3p4 = np.linalg.norm(vect_p3p4)

            # fonction pour distance C → droite (p3,p4)
            def dist_c_to_seg(C, p3, p4):
                A = np.array([p3[1] - p4[1], p4[0] - p3[0]])  # vecteur normal (x,z)
                num = abs(A[0] * (C[0] - p3[0]) + A[1] * (C[1] - p3[1]))
                den = np.linalg.norm(A)
                return num / den if den > 1e-12 else 0.0

            # boucle "tant que" la tangence n’est pas atteinte
            dist_cc = dist_c_to_seg(C, p3_xz, p4_xz)
            while dist_cc <= r_corde:
                # impose dist = dc/2
                # on fait varier θ_total pour garder la distance tangente
                theta_total += 0.0008  # petit incrément angulaire
                p4 = p3 + np.array([-D*np.cos(theta_total), 0.0, D*np.sin(theta_total)])
                p4_xz = np.array([p4[2], p4[0]])
                dist_cc = dist_c_to_seg(C, p3_xz, p4_xz)

                if abs(dist_cc - r_corde) < 1e-5:
                    break

            # on déduit θ3
            theta3 = theta_total - th2
            theta3 = np.arctan2(np.sin(theta3), np.cos(theta3))

        # mise à jour finale
        p4 = p3 + np.array([-D*np.cos(theta_total), 0.0, D*np.sin(theta_total)])
        p4_xz = np.array([p4[2], p4[0]])
        dist_cc = np.linalg.norm(C - p4_xz)

        # calcul des arcs pour affichage
...
if __name__ == "__main__":
    ani, frames, theta3_sig, theta_total_sig, dist_cc = animate_robot_full()
